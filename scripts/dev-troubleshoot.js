...
class DevTroubleshooter {
  ...
  async fixMissingNodeModules() {
    ...
    if (!fs.existsSync(nodeModulesPath)) {
      ...
      if (this.autoFix) {
        ...
        const result = await this.runCommand('npm install', 'Dependency installation');
        if (result.success) {
      ...
      if (netstatResult.trim()) {
        ...
        if (this.autoFix) {
          ...
          try {
            ...
            this.log(`Could not free port automatically. Manual intervention required.`, 'error');
            return false;
          }
        } else {
    ...
    if (fs.existsSync(envExamplePath) && !fs.existsSync(envLocalPath)) {
      ...
      if (this.autoFix) {
        ...
        try {
          ...
          this.log('Remember to update .env.local with your actual values!', 'warning');
          return true;
        } catch (error) {
  ...
  async fixTypeScriptIssues() {
    ...
    if (!result.success) {
      this.log('TypeScript errors detected. Review and fix manually.', 'warning');
  ...
  generateReport() {
    ...
    console.log('4. Run this troubleshooter with --fix-auto for automatic fixes');
    
    this.log('\n📞 NEED MORE HELP?', 'info');
    console.log('- Check DEVELOPMENT.md for detailed setup instructions');
    console.log('- Run "node scripts/dev-health-check.js" for comprehensive diagnostics');
    console.log('- Review the Next.js documentation for advanced troubleshooting');
  }

  async run() {
    this.log(`🔧 Starting development troubleshooter... (auto-fix: ${this.autoFix})`, 'info');
