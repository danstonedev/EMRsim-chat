...
class DevHealthChecker {
  ...
  checkPackageJson() {
    ...
    const packageJson = JSON.parse(fs.readFileSync(path.join(this.rootDir, 'package.json')));
    ...
    if (nextVersion && !nextVersion.startsWith('15.')) {
      this.addWarning(`Next.js version ${nextVersion} - consider upgrading to 15.x`);
    }
  }
  ...
  checkNextConfig() {
    ...
    const configContent = fs.readFileSync(path.join(this.rootDir, 'next.config.js'), 'utf8');
    ...
    if (!configContent.includes('swcMinify')) {
      this.addWarning('SWC minification not enabled - consider adding for faster builds');
    }
  ...
  checkTypeScriptConfig() {
    ...
    if (this.checkFileExists('tsconfig.json')) {
      const tsConfig = JSON.parse(fs.readFileSync(path.join(this.rootDir, 'tsconfig.json')));
  ...
  checkEnvironmentFiles() {
    ...
    if (this.checkFileExists('.env.local', false)) {
      ...
      this.addWarning('No .env.local file - copy from .env.local.example if needed');
    }
    
    // Check gitignore
    if (this.checkFileExists('.gitignore')) {
      const gitignore = fs.readFileSync(path.join(this.rootDir, '.gitignore'), 'utf8');
  ...
  async checkDevServerHealth() {
    ...
    if (!isPortAvailable) {
      this.addWarning(`Port ${port} is already in use - dev server might already be running`);
    } else {
  ...
  generateReport() {
    ...
    if (this.issues.length === 0 && this.warnings.length === 0) {
      this.log('🎉 All checks passed! Your development environment is healthy.', 'success');
    } else {
