...
export type PTCaseId = 'lowBackPain' | 'aclRehab' | 'rotatorCuff' | 'strokeGait' | 'ankleSprain'

const CASES: Record<PTCaseId, PatientPromptOptions> = {
  lowBackPain: {
    ...
    context: {
      ...
      associatedSymptoms: 'stiffness in the morning, improves with light movement',
  ...
  aclRehab: {
    profile: { name: 'Jordan', age: 19, sex: 'male', occupation: 'college athlete' },
    context: {
      chiefComplaint: 'knee stiffness and weakness after ACL reconstruction 6 weeks ago',
      onset: 'post-surgical, improving but still limited',
      severity: 'mild (2–4/10) with certain movements',
      associatedSymptoms: 'fear of reinjury, occasional swelling after long walks',
      meds: 'none daily',
      allergies: 'NKDA'
    },
    tone: 'anxious',
  ...
  strokeGait: {
    ...
    context: {
      chiefComplaint: 'fatigue and imbalance while walking after stroke 4 months ago',
  ...
  ankleSprain: {
    profile: { name: 'Riley', age: 28, sex: 'non-binary', occupation: 'barista' },
    context: {
      chiefComplaint: 'right ankle pain and swelling after inversion injury 10 days ago',
...
export function getPTPrompt(caseId?: string): string {
  const key = (caseId as PTCaseId) && CASES[(caseId as PTCaseId)] ? (caseId as PTCaseId) : 'lowBackPain'
