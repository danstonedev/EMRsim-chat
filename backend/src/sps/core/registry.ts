...
export class SPSRegistry {
  ...
  addChallenges(items: ScreeningChallenge[]) {
    items.forEach(i => {
      zScreeningChallenge.parse(i);
      this.screening[i.id] = i;
    });
    return this;
  }
  addSpecialQuestions(items: SpecialQuestion[]) {
    items.forEach(i => {
      zSpecialQuestion.parse(i);
      this.specials[i.id] = i;
    });
    return this;
  }
  addPersonas(items: PatientPersona[]) {
    items.forEach(i => {
      zPersona.parse(i);
      this.personas[i.patient_id] = i;
    });
    return this;
  }
  addScenarios(items: ClinicalScenario[]) {
    items.forEach(i => {
      zClinicalScenario.parse(i);
      this.scenarios[i.scenario_id] = i;
    });
    return this;
  }

  getScenarioChallenges(s: ClinicalScenario) {
    const ids = s.screening_challenge_ids ?? [];
    return ids.map(id => this.screening[id]).filter(Boolean);
  }
  getScenarioSpecials(s: ClinicalScenario) {
    const ids = new Set(s.special_question_ids ?? []);
    return Object.values(this.specials).filter(sq => ids.has(sq.id));
  }
  ...
  composeActiveCase(personaId: string, scenarioId: string): ActiveCase {
    ...
    if (g.sex_required && persona.demographics.sex.toLowerCase() !== g.sex_required)
      console.warn('Persona sex != scenario requirement');
