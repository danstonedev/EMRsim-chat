...
export const TONE_MAP: Record<string, 'friendly' | 'guarded' | 'disinterested' | 'worried' | 'irritable' | 'stoic' | 'optimistic'> = {
  guarded: 'guarded',
  cautious: 'guarded',
  skeptical: 'guarded',
  firm: 'guarded',
 ...
 */
export function mapTone(input: unknown): 'friendly' | 'guarded' | 'disinterested' | 'worried' | 'irritable' | 'stoic' | 'optimistic' {
...
export function normalizePersona(input: PersonaNormalizationInput): PatientPersona | null {
  const { raw, instructions, subjective, plan } = input;
  ...
  const id = String(
    ...
    raw.persona_id || 
    raw.id || 
    `persona_${Math.random().toString(36).slice(2, 8)}`
  );
  
  // Extract demographics
  const age = typeof raw.age === 'number' && Number.isFinite(raw.age) 
    ? raw.age 
    : Number(raw.age) || 30;
  ...
  const demographics: PatientPersona['demographics'] = {
    ...
    primary_language: Array.isArray(raw.cultural_linguistic_notes) && raw.cultural_linguistic_notes.length
  ...
  const social_context = Array.isArray(raw.support_system) && raw.support_system.length
  ...
  const surgicalHistory = mergeStringArrays(
    raw.past_surgical,
    subjective?.medical_history?.past_surgical
  );
  const medications = mergeStringArrays(
    raw.medications,
    subjective?.medical_history?.medications
  );
  const imagingHistory = subjective?.medical_history?.imaging_workup
    ? String(subjective.medical_history.imaging_workup)
    : undefined;

  const history_snapshot = (medicalHistory || surgicalHistory || medications || imagingHistory)
    ? {
  ...
  const quirks = Array.isArray(raw.quirks) 
    ? raw.quirks.map((q: any) => String(q)).filter(Boolean) 
    : undefined;
...
export function normalizeObjectiveFinding(
  ...
  const numeric = findings && typeof findings.numeric === 'object' 
    ? findings.numeric 
    : undefined;
  const qualitative = Array.isArray(findings?.qualitative) 
    ? findings.qualitative.map((s: any) => String(s)) 
    : undefined;
  const binary = findings && typeof findings.binary_flags === 'object' 
    ? findings.binary_flags 
    : undefined;
 ...
 */
export function normalizeObjectiveCatalog(
  objective: any,
  region: string,
  scenarioId: string
): ObjectiveFinding[] {
