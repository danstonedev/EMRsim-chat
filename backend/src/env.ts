...
const envSchema = z.object({
  ...
  SPS_ENABLED: z.string().transform(val => val === 'true').default('true'),
  VOICE_DEBUG: z.string().transform(val => val === 'true').default('false'),
  BANNERS_ENABLED: z.string().transform(val => val !== 'false').default('true'),
  
  // OpenAI Configuration
  OPENAI_API_KEY: z.string().min(1, 'OPENAI_API_KEY is required'),
  OPENAI_REALTIME_MODEL: z.string().default('gpt-realtime-2025-08-28'),
  OPENAI_TEXT_MODEL: z.string().default('gpt-4o'),
  OPENAI_TTS_VOICE: z.enum([
  ...
  REALTIME_VAD_THRESHOLD: z.string().regex(/^\d*\.?\d+$/).default('0.30').transform(Number),
  ...
  SPEAKER_DIARIZATION_ENABLED: z.string().transform(val => val === 'true').default('false'),
  SPEAKER_DIARIZATION_PROVIDER: z.enum([
    'openai', 'azure', 'gcp', 'aws', 'pyannote', 'nemo', 'none'
  ]).default('none'),
  SPEAKER_DIARIZATION_MAX_SPEAKERS: z.string().regex(/^\d+$/).default('2').transform(Number),
  
  SPEAKER_ID_ENABLED: z.string().transform(val => val === 'true').default('false'),
  SPEAKER_ID_PROVIDER: z.enum(['azure_speaker', 'custom', 'none']).default('none'),
  SPEAKER_ID_THRESHOLD: z.string().regex(/^\d*\.?\d+$/).default('0.7').transform(Number),
