...
describe('ConversationController', () => {
  ...
  it('sends text payloads when the active data channel is open', async () => {
    ...
    ;(controller as any).activeChannel = {
      readyState: 'open',
      send,
    }
  ...
  it('records diagnostics once debug is enabled at runtime', () => {
    ...
    ;(controller as any).emitDebug({ t: new Date().toISOString(), kind: 'info', src: 'app', msg: 'before enable' })
  ...
  ;(controller as any).emitDebug({ t: new Date().toISOString(), kind: 'info', src: 'app', msg: 'after enable' })
  ...
  it('refreshInstructions forwards new payloads to the realtime channel', async () => {
    const controller = new ConversationController({ debugEnabled: false })
    const send = vi.fn()
    ;(controller as any).sessionId = 'session-123'
    ;(controller as any).sessionReady = true
    ;(controller as any).activeChannel = { readyState: 'open', send }
    ...
    expect(payload.response.instructions).toContain('assess vitals')
  ...
  it('dedupes refreshInstructions calls based on instructions, phase, and outstanding gates', async () => {
    ...
    const spy = vi.spyOn(api, 'getVoiceInstructions')
  ...
  it('waits for session.updated before sending initial response.create payload', async () => {
    const controller = new ConversationController({ debugEnabled: false })
    const send = vi.fn()
    ...
    const responseCreateCallsBeforeAck = send.mock.calls
      .map(([raw]) => JSON.parse(raw))
      .filter((payload) => payload?.type === 'response.create')

    expect(spy).not.toHaveBeenCalled()
    expect(responseCreateCallsBeforeAck.length).toBe(0)
    ...
    for (let i = 0; i < 5 && (controller as any).instructionSyncInFlight; i += 1) {
      await Promise.resolve()
    }

    const responseCreateCallsAfterAck = send.mock.calls
      .map(([raw]) => JSON.parse(raw))
      .filter((payload) => payload?.type === 'response.create')

    expect(spy).toHaveBeenCalledTimes(1)
    expect(responseCreateCallsAfterAck.length).toBe(1)
