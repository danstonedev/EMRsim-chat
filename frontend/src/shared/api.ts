import type { MediaReference } from './ConversationController'
...
export const api = {
  ...
  async getVoiceInstructions(sessionId: string, options?: { phase?: string | null; gate?: Record<string, unknown> | null }): Promise<{ instructions: string; phase?: string; outstanding_gate?: string[] }>{
  ...
  async saveSpsTurns(sessionId: string, turns: Array<{ role: 'user'|'assistant'; text: string; channel?: 'text'|'audio'; timestamp_ms?: number }>): Promise<{ ok: boolean; saved: number; received?: number; duplicates?: number }> {
  ...
  async createSession(persona_id: string, scenario_id: string): Promise<{session_id:string; sps_session_id?:string; phase?:string; gate?:any; gate_state?:string}> {
  ...
  async getVoiceToken(sessionId: string, overrides?: { voice?: string; input_language?: string; model?: string; transcription_model?: string; reply_language?: string }): Promise<{ rtc_token: string; model: string; tts_voice: string; opts?: { expires_at?: string }; persona?: { id: string; display_name: string | null; speaking_rate: string | null; voice_id: string | null } }> {
    ...
    if (!r.ok) {
      ...
      try { const obj = JSON.parse(txt); if (obj && obj.error) code = String(obj.error) } catch {}
  ...
  async relayTranscript(
    ...
    console.log('[API] 🚀 relayTranscript called:', {
      url,
    ...
    console.log('[API] 📡 relayTranscript response:', {
      status: r.status,
      ok: r.ok,
  ...
  async listSpsScenarios(): Promise<Array<{
    scenario_id: string;
    title: string;
    region: string;
  ...
  async getSpsScenarios(): Promise<Array<{
    scenario_id: string;
    title: string;
    region: string;
    difficulty?: string | null;
  ...
  async getSpsPersonas(): Promise<Array<{ id: string; display_name: string | null; headline: string | null; age?: number | null; sex?: string | null; voice?: string | null; tags?: string[] }>> {
